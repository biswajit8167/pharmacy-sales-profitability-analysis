# pharmacy-sales-profitability-analysis
# Pharmacy Sales & Profitability Analysis Dashboard

## Project Overview

This project analyzes pharmacy sales, profitability, promotions, regional performance, and product analytics using Power BI.

The dashboard helps identify:

- Revenue trends
- Profitability insights
- High-performing pharmacies
- Product contribution
- Promotion effectiveness
- Geographic sales patterns
- Regional performance

---

# Tools & Technologies

- Power BI
- Power Query
- DAX
- Data Modeling
- SQL Concepts
- Business Intelligence

---

# Dashboard Pages

## 1. Executive Overview

- Revenue KPI
- Margin KPI
- Units Sold
- Country Performance
- Brand Analysis
- Pharmacy Ranking

![Overview Dashboard](screenshots/overview.png)

---

## 2. Regional & Pharmacy Analysis

- Pharmacy Drill-down Analysis
- Revenue vs Margin Analysis
- Store Size Performance
- Pharmacy Ranking
- Regional Contribution

![Regional Dashboard](screenshots/regional_analysis.png)

---

## 3. Product Performance & Profitability

- Product Revenue Analysis
- Brand Performance
- Category Contribution
- Product Ranking
- Margin Analysis

![Performance Dashboard](screenshots/performance_analysis.png)

---

## 4. Promotion & Campaign Analysis

- Promoted vs Non-Promoted Sales
- Campaign Impact
- Promotion Profitability
- Category-wise Promotion Analysis

![Promotion Dashboard](screenshots/promotion_analysis.png)

---

## 5. Geographical Analysis

- Country Revenue Map
- Top Cities
- Top Regions
- Geographic Revenue Distribution

![Geography Dashboard](screenshots/geography_analysis.png)

---

# Key Business Insights

- Revenue increased significantly across major pharmacy regions.
- Prescription products contribute the highest profitability.
- Promotions improve sales volume but reduce margin percentage.
- Urban pharmacies outperform rural pharmacies in total revenue.
- Germany is the strongest revenue-generating country.

---

# Data Model

The project follows a Star Schema Model:

- FactSales
- DimDate
- DimProduct
- DimPharmacy

---

# Skills Demonstrated

- Data Cleaning
- Data Transformation
- Data Modeling
- DAX Calculations
- Dashboard Design
- Business Storytelling
- KPI Development
- Interactive Reporting

---

# Author

Biswajit Maity
